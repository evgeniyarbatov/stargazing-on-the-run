...
def test_get_timezone_from_points_empty() -> None:
    assert get_timezone_from_points([]) == "UTC"


def test_get_timezone_from_points_new_york() -> None:
    points = [
...
def test_add_view_angles_includes_overhead() -> None:
...
def test_add_view_angles_reuses_points_when_needed() -> None:
    point = Point(time="t1", timestamp="100", lat=1.0, lon=2.0, az=350.0, alt=0.0)
